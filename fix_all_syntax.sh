...
echo "🔧 Comprehensive Syntax Fix - Fixing ALL broken files..."

# Fix horses.py (the current error)
echo "📝 Fixing app/api/horses.py..."
cat > app/api/horses.py << 'HORSESPY'
...
echo "✅ Fixed horses.py"

# Restart the API to pick up the fix
echo "🔄 Restarting API..."
docker-compose restart api

echo "⏱️ Waiting for API to start..."
sleep 10
...
docker-compose exec frontend curl -s http://172.18.0.5:8000/health
